...
const Sidebar = ({
  ...
  const getAvatarText = (username, imageUrl) => {
    if (imageUrl) return { type: "image", src: imageUrl };
    if (!username.trim()) return { type: "text", initials: "?" };
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="p-4 border-b border-white relative">
          {/* Close button mobile */}
          <button
            onClick={() => setIsMobileOpen(false)}
          ...
          <div className={`flex items-center ${isCollapsed ? "justify-center" : "space-x-4"}`}>
            <img
              src={isCollapsed ? logok : logo}
              alt="Logo"
              className={`${isCollapsed ? "h-10" : "h-16"} w-auto`}
            />
          </div>
          ...
          <button
            ...
            className={`mt-4 w-full bg-blue-900 hover:bg-blue-600 rounded-xl py-3 px-2 flex items-center justify-center space-x-2 shadow-lg
              ${isCollapsed ? "mt-6" : ""}
            `}
          >
            <FiPlus />
            {!isCollapsed && <span>Nouveau CV</span>}
          </button>
        </div>
        ...
        <nav className="flex-1 py-6 px-4 space-y-2">
          {menuItems.map((item, index) => (
            <NavLink
              ...
              onClick={() => setIsMobileOpen(false)}
              className={({ isActive }) =>
                `flex items-center space-x-3 py-3 px-4 rounded-xl transition-all
                ${isActive
        ...
        <div className="p-2 hidden md:block">
          <button
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="w-full flex justify-center py-2 hover:bg-blue-800 rounded-xl"
          >
            {isCollapsed ? <FaArrowCircleRight /> : <FaArrowCircleLeft />}
          </button>
        </div>
        ...
        <div className="p-4 border-t border-blue-700 space-y-3">
          <div className={`flex items-center ${isCollapsed ? "justify-center" : "space-x-3"}`}>
            <div className="w-10 h-10 bg-blue-700 rounded-full flex items-center justify-center overflow-hidden">
              {avatar.type === "image" ? (
          ...
          <button
            ...
            className="w-full flex items-center space-x-3 py-3 px-4 hover:bg-blue-800 rounded-xl"
          >
            <FiLogOut />
