...
const Board = () => {
  ...
  useEffect(() => {
    if (!userId) return;
    setLoadingTotalCVs(true);
    axiosInstance.get(`/cvs/user/${userId}`)
      .then(res => setTotalCVs(res.data.total ?? 0))
      .catch(() => setTotalCVs(0))
      .finally(() => setLoadingTotalCVs(false));
  }, [userId]);

  useEffect(() => {
    if (!userId) return;
    setLoadingLastCv(true);
    axiosInstance.get(`/cvs/users/${userId}/last`)
      .then(res => setLastCvTitle(res.data.title || "..."))
      .catch(() => setLastCvTitle("..."))
      .finally(() => setLoadingLastCv(false));
  }, [userId]);

  useEffect(() => {
    setLoadingStats(true);
    axiosInstance.get(`/cvs/in-progress/${userId}`)
      .then(res =>
        setStats({
          completed_cvs: res.data.completed_cvs ?? 0,
          in_progress_cvs: res.data.in_progress_cvs ?? 0
        })
      )
      .catch(() => setStats({ completed_cvs: 0, in_progress_cvs: 0 }))
  ...
  const openConfirmPopup = (message, onConfirm, onCancel) => {
    setConfirmPopup({ visible: true, message, onConfirm, onCancel });
  };
  ...
  return (
    <div className="min-h-screen bg-white p-4 pt-16 md:pt-4 border-white">
      {/* <div className="min-h-screen bg-white p-4 border-blue-900"> */}
      ...
      <div className="max-w-7xl mx-auto mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
          ...
          <div className="relative group">
            <div className="relative bg-white rounded-2xl p-6 shadow-xl border border-emerald-100">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-gray-500 text-sm font-medium">Dernier CV modifié</span>
                  <span className="block text-xl font-semibold text-gray-900 mt-2 truncate max-w-[120px]">
          ...
          <div className="relative group">
            <div className="relative bg-white rounded-2xl p-6 shadow-xl border border-amber-100">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-gray-500 text-sm font-medium">CV Complétés</span>
                  <span className="block text-3xl font-bold text-gray-900 mt-2">
          ...
          <div className="relative group">
            <div className="relative bg-white rounded-2xl p-6 shadow-xl border border-violet-100">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-gray-500 text-sm font-medium">En progression</span>
                  <span className="block text-3xl font-bold text-gray-900 mt-2">
