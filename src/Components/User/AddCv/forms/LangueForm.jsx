...
const LangueForm = ({ data = [], onUpdate }) => {
  ...
  const emitUpdate = (updatedLangues) => {
    const filtered = updatedLangues
      ...
      .map(({ nom, nom_custom, niveau }) => ({
        name: nom === "Autre" ? nom_custom.trim() : nom.trim(),
        level: niveau || "", // On renvoie la valeur telle qu’elle est
      }));
    if (onUpdate) onUpdate(filtered);
  };
  ...
  const removeLangue = (index) => {
    ...
    setLangues(finalList);
    emitUpdate(finalList);
  };
  ...
  const getNiveauText = (niveau) => {
    ...
    return niveaux[niveau] || "Niveau non défini";
  };
  ...
  const getNiveauColor = (niveau) => {
    ...
    return colors[niveau] || "bg-gray-200 text-gray-600";
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg border">
              <div className="flex justify-between items-center mb-2">
                <span className="font-medium text-gray-700">
                  {langue.nom === "Autre"
                    ? langue.nom_custom || "Langue personnalisée"
                    : langue.nom || langue.name|| "Langue non définie"}
                </span>
      ...
      <button
        onClick={addLangue}
        className="w-full flex items-center justify-center space-x-2 p-4 border-2 border-dashed border-gray-300 rounded-xl text-gray-600 hover:border-blue-500 hover:text-blue-500"
      >
        <Plus size={20} />
        <span className="font-medium">Ajouter une langue</span>
      </button>
