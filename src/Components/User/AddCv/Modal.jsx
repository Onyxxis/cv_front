...
import React, { useState, useRef, forwardRef, useImperativeHandle } from 'react';
import { createPortal } from 'react-dom';
import { FiX, FiUpload, FiFileText, FiCheck, FiLoader } from 'react-icons/fi';
import { useNavigate } from 'react-router-dom';
import axios from "../../../api/axiosInstance"; 
...
export const Modal = forwardRef((props, ref) => {
    ...
    const handleFileUpload = (e) => {
        ...
        if (!validTypes.includes(file.type) && !file.name.match(/\.(pdf|doc|docx)$/i)) {
            setFileError('Veuillez sélectionner un fichier PDF ou Word valide.');
            return;
        }
    ...
    const importCV = async (file) => {
        try {
            ...
            console.error("IMPORT ERROR", err);
            setFileError("Erreur lors de l'import du CV. Veuillez réessayer.");
        }
    ...
    const handleSubmit = async (e) => {
        ...
        // Validation spécifique pour l'upload
        if (formData.actionType === "upload" && !formData.uploadedFile) {
    ...
    const modalContent = (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fadeIn">
            <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full mx-auto overflow-hidden animate-scaleIn max-h-[90vh] flex flex-col">
                
                {/* HEADER */}
                <div className="relative p-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50 flex-shrink-0">
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 to-indigo-600"></div>
                    <div className="flex items-center justify-between">
                        <div>
                            <h2 className="text-xl font-bold text-gray-900">Nouveau Curriculum Vitae</h2>
                ...
                {/* FORMULAIRE - Conteneur scrollable */}
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button
                                    type="button"
                                    onClick={() => handleInputChange('actionType', 'upload')}
                                    className={`p-4 border-2 rounded-xl flex flex-col items-start transition-all duration-300 min-h-[100px]
                                        ${formData.actionType === 'upload'
                                            ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-200 shadow-lg'
                                ...
                                >
                                    ...
                                    <span className={`text-sm font-semibold ${
                                        ...
                                        Importer un CV (disponible bientot)
                                    </span>
                        ...
                        {formData.actionType === 'upload' && (
                            <div className="animate-slideDown space-y-3">
                                ...
                                {/* Info-bulle aide */}
                                {!formData.uploadedFile && !fileError && (
                                    <p className="text-xs text-gray-500 bg-blue-50 p-3 rounded-lg">
                                        💡 <strong>Conseil :</strong> Importez votre CV existant pour gagner du temps. 
                                        Nous analyserons automatiquement son contenu.
                                    </p>
                                )}
                            </div>
                        )}
                ...
                <div className="border-t border-gray-100 bg-white p-6 flex-shrink-0">
                    <div className="flex space-x-3">
                        ...
                        <button
                            ...
                            disabled={(formData.actionType === 'upload' && !formData.uploadedFile) || loadingImport}
                            className="flex-1 py-3.5 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-semibold shadow-lg hover:shadow-xl disabled:hover:shadow-lg flex items-center justify-center gap-2"
                        >
                            {loadingImport ? (
                                <>
